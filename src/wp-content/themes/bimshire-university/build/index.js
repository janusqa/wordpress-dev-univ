...
class MyNotes {
  ...
  async createNote(e) {
    ...
    try {
      ...
      document.querySelector('#my-notes').insertAdjacentHTML('afterbegin', ` <li data-id="${json.id}" class="fade-in-calc">
  ...
  getNote(element, target) {
    ...
    while (currentElement && currentElement !== document) {
      if (currentElement.tagName.toLowerCase() === target && currentElement.hasAttribute('data-id')) {
...
class Search {
  ...
  events() {
    ...
    this.searchField.addEventListener('keyup', e => this.processSearchTerm());
  }
  openOverlay() {
  ...
  async getResults() {
    ...
    if (this.searchField.value.trim().length > 3) {
      const response = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].get(`${apiUrl}/search?term=${this.searchField.value}`);
      const results = response.data;
      this.searchResults.innerHTML = `
                <div class="row">
                    <div class="one-third">
                        <h2 class="search-overlay__section-title">General Information</h2>
                        ${results['posts'].length ? '<ul class="link-list min-list">' : '<p>No results found</p>'}
                        ${results['posts'].map(result => `<li><a href="${result.permalink}">${result.title}</a> ${result.postType == 'post' ? ` by ${result.authorName}` : ''}</li>`).join('')}
                    ...
                    <div class="one-third">
                        <h2 class="search-overlay__section-title">Campuses</h2>
                            ${results['campuses'].length ? '<ul class="link-list min-list">' : '<p>No results found</p>'}
                            ${results['campuses'].map(result => `<li><a href="${result.permalink}">${result.title}</a></li>`).join('')}
                            ${results['campuses'].length ? '</ul>' : ''}                                                        
                        <h2 class="search-overlay__section-title">Events</h2>
                            ${results['events'].length ? '' : '<p>No results found</p>'}
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = globalThis["webpackChunkbimshire_university"] = globalThis["webpackChunkbimshire_university"] || [];
